...
describe('Render Github routes End to End Tests', () => {
  ...
  beforeAll(async (done) => {
    ...
    await page.goto('http://localhost:3000/github/all-repos', {
  ...
  it(`GET all commits of repo`, async () => {
    await page.goto(
      'http://localhost:3000/github/repo-all-commits?repoName=christiangracia.com3.0&numberOfCommits=250',
      { waitUntil: 'networkidle2' },
    );
    const data = await page.evaluate(
      () => document.querySelector('body').innerText,
