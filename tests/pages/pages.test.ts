...
pages.forEach((page: Page) => {
  ...
  describe('Rendered Views Tests', () => {
    ...
    it(`GET rendered Title is equal to file on server's title`, async () => {
      const response = await supertest(app).get(page.baseUrl);
      const titleFound = htmlHelper.getHTmlElements(response.text, 'title')[0]
        .textContent;
      Logger.info(
        `--------------------- rendered title: ${titleFound} | title on server: ${title} ---------------------`,
      );
